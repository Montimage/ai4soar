...
class PlaybookService:
    ...
    def get_playbook_results(self, execution_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            execution_data: Execution data containing execution_id and authorization
            
        Returns:
            Execution results
            
        Raises:
            PlaybookError: If fetching results fails
        """
        execute_url = f"{self.shuffle_config.api_base_url}/streams/results"
        
        try:
            response = requests.post(execute_url, headers=self.headers, json=execution_data)
            response.raise_for_status()
